...
import { defaultStopTimeout, ffmpegCoreUrl, silenceRemoveCommand, whisperApiEndpoint } from "./configs.ts";
import { UseWhisperConfig, UseWhisperHook, UseWhisperTimeout, UseWhisperTranscript } from "./types";
...
export const useWhisper: UseWhisperHook = (config) => {
	...
	const [transcript, setTranscript] = useState<UseWhisperTranscript>(defaultTranscript);
		...
		try {
			if (streaming && recorder.current) {
				...
				if (recorderState === "recording") {
					...
					console.log("Send data to whisper");
					const text = await onWhispered(file);
